...
import { DefaultServerOptions, EventInternal } from "./constants.js";
import {
  closeSocket,
  omitValues,
  runSafeAsync,
...
export class UltraVNCRepeater {
  ...
  constructor(options: Partial<VNCRepeaterOptions>) {
    ...
    if (this._options.logger) {
      ...
      const options = {
        level: this._options.logLevel ?? "debug",
      };
  ...
  getPendingConnections() {
    return Array.from(this._pendingConnections.entries());
  }

  getActiveConnections() {
    return Array.from(this._activeConnections.entries());
  }
  ...
  protected async _onNewServer({
    ...
    if (!this._canHookupConnection(id)) {
      this._logger.info(`refusing extra vnc server for ID:${id}.`);
    ...
    if (connection.client) {
      this._logger.info(
        `hooking up new vnc server with existing vnc client for ID:${id}.`,
      );
    ...
    this._logger.info(
      `closing and deleting previous vnc server with ID:${id}.`,
    );
    runSafeAsync(() => this._closeAndDeletePendingConnection(id));
    this._logger.info(`storing new vnc server with ID:${id}.`);
    this._pendingConnections.set(id, { server: serverSocket, client: null });
  }
  ...
  protected async _onNewClient({
    ...
    if (!id) {
      ...
      this._logger.info(
        `making vnc client connection directly to vnc server host='${host}' port='${port}'.`,
      );
    ...
    if (connection.client) {
      if (!this._canHookupConnection(id)) {
        this._logger.info(`refusing extra vnc client for ID:${id}.`);
        await closeSocket(clientSocket);
        return;
      }
      this._logger.info(
        `closing and deleting previous vnc client with ID:${id}.`,
      );
      await closeSocket(connection.client);
    }

    this._logger.info(`storing new vnc client with ID:${id}.`);
    connection.client = clientSocket;

    if (connection.server) {
      this._logger.info(
        `hooking up new vnc client with existing vnc server for ID:${id}.`,
      );
      this._pendingConnections.delete(id);
      this._addActiveConnection(id, {
  ...
  protected async _initServerGateway() {
    ...
    this._server.on(
      ...
      safeAsync({
        ...
        onError: (err) =>
          this._logger.warn(`unexpected error during server closing`, {
            err,
          }),
      }),
    );
  ...
  protected async _registerCleanups() {
    ...
    const cleanup = () => {
      ...
      if (signalForceShutdown) {
        ...
        wait(this._options.killTimeout! * 1000).then(() => process.exit(0));
      }
      signalForceShutdown = true;
    };
    ["SIGINT", "SIGTERM"].forEach((signal) => {
      process.on(signal, cleanup);
      this._client?.once("close", () => process.off(signal, cleanup));
      this._server?.once("close", () => process.off(signal, cleanup));
    });
  }
